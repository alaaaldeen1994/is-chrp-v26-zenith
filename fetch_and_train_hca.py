...
def main():
    print("="*60)
    print("ðŸŒ CONNECTING TO HUMAN CELL KAGAWEA (HCA) REPOSITORY...")
    print("="*60)

    # 1. Fetch Real Data
    print("â¬‡ï¸  Downloading Heart Cell KAGAWEA (Subsampled)...")
    try:
        # Try to get the specific Heart Atlas subset
        adata = scvi.data.heart_cell_atlas_subsampled()
        dataset_name = "Heart Cell Atlas"
    except Exception as e:
        print(f"âš ï¸  Heart KAGAWEA not found: {e}")
        print("ðŸ”„ Falling back to PBMC3k (Standard Human Reference)...")
        adata = sc.datasets.pbmc3k()
        dataset_name = "PBMC 3k (Human)"

    print(f"âœ… Loaded {dataset_name}: {adata.n_obs} cells x {adata.n_vars} genes")

    # 2. Preprocessing (Standard scRNA-seq pipeline)
    print("ðŸ”¬ Preprocessing raw counts...")
    ...
    print(f"ðŸ§¬ Selected 2,000 Highly Variable Genes for Manifold Learning")

    # 3. Setup scVI
    print("ðŸ¤– Initializing scVI Variational Autoencoder...")
    scvi.model.SCVI.setup_anndata(adata, layer="counts")
    model = scvi.model.SCVI(adata, gene_likelihood="nb", n_latent=16) # Match simulation 16D

    # 4. Train
    print(f"ðŸ§  Training model on Real Human Data ({EPOCHS} epochs)...")
    ...
    print("\n" + "="*60)
    print(f"âœ… SUCCESS: Real {dataset_name} Integrated.")
