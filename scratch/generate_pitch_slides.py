...
class PitchDeckPDF(FPDF):
    ...
    def draw_badge(self, x, y, text, bg_color, text_color, width=35, height=6, font_size=7):
        self.set_fill_color(*bg_color)
...
def generate_deck():
    ...
    pdf.rect(25, 40, 10, 10, 'F', round_corners=True, corner_radius=2.5)
    pdf.set_fill_color(5, 150, 105) # Emerald
    pdf.rect(30, 45, 10, 10, 'F', round_corners=True, corner_radius=2.5)
    
    pdf.set_xy(45, 43)
    pdf.set_font(pdf.get_font_name(True), 'B', 18)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(100, 8, 'NILUS LAB', 0, 1, 'L')
    
    # Main Title
    pdf.set_xy(25, 75)
    pdf.set_font(pdf.get_font_name(True), 'B', 32)
    pdf.set_text_color(255, 255, 255)
    pdf.multi_cell(220, 13, 'Reversing Cardiac Ageing\nWithout Oncogene Risk')
    
    # Subtitle
    pdf.set_xy(25, 115)
    pdf.set_font(pdf.get_font_name(), '', 13)
    pdf.set_text_color(148, 163, 184) # Slate 400
    pdf.multi_cell(220, 7.5, 'Generative cellular reprogramming therapies targeting cardiomyopathy and heart failure.\nPowered by the Zenith v28.0 GOLD single-cell foundation model.')
    
    # Accelerator track details
    pdf.set_xy(25, 165)
    pdf.set_font(pdf.get_font_name(True), 'B', 9.5)
    ...
    pdf.draw_badge(22, 45, 'CLINICAL RISK & LIMITS', bg_color=(244, 63, 94), text_color=(255, 255, 255), width=45, height=6)
    ...
    for title, desc in bullets_problem:
        ...
        pdf.multi_cell(106, 4.5, desc)
        y_cursor += 28
        
    # Right Column (The Solution) - Indigo Card
    pdf.draw_card(154, 38, 128, 152, bg_color=(239, 246, 255), border_color=(191, 219, 254), r=4)
    pdf.draw_badge(161, 45, 'ZENITH PLATFORM SOLUTION', bg_color=(37, 99, 235), text_color=(255, 255, 255), width=50, height=6)
    ...
    pdf.set_fill_color(79, 70, 229) # Indigo 600
    pdf.ellipse(22, 45, 8, 8, 'F')
    pdf.set_xy(22, 45)
    pdf.set_font(pdf.get_font_name(True), 'B', 9)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(8, 8, '1', 0, 0, 'C')
    ...
    pdf.ellipse(22 + col_width + spacing, 45, 8, 8, 'F')
    pdf.set_xy(22 + col_width + spacing, 45)
    pdf.set_font(pdf.get_font_name(True), 'B', 9)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(8, 8, '2', 0, 0, 'C')
    
    pdf.set_xy(33 + col_width + spacing, 46)
    pdf.set_font(pdf.get_font_name(True), 'B', 12)
    pdf.set_text_color(15, 23, 42)
    ...
    pdf.set_fill_color(79, 70, 229)
    pdf.ellipse(22 + 2*(col_width + spacing), 45, 8, 8, 'F')
    pdf.set_xy(22 + 2*(col_width + spacing), 45)
    pdf.set_font(pdf.get_font_name(True), 'B', 9)
    pdf.set_text_color(255, 255, 255)
    pdf.cell(8, 8, '3', 0, 0, 'C')
    
    pdf.set_xy(33 + 2*(col_width + spacing), 46)
    pdf.set_font(pdf.get_font_name(True), 'B', 12)
    pdf.set_text_color(15, 23, 42)
    ...
    pdf.draw_badge(22, 45, 'MARKET POSITIONING', bg_color=(79, 70, 229), text_color=(255, 255, 255), width=35, height=6)
    ...
    pdf.set_xy(173, 52)
    pdf.set_font(pdf.get_font_name(True), 'B', 28)
    pdf.set_text_color(79, 70, 229) # Indigo
    ...
    pdf.draw_badge(22, 45, 'THE TEAM', bg_color=(79, 70, 229), text_color=(255, 255, 255), width=20, height=6)
    ...
    pdf.draw_badge(150, 45, 'DEVELOPMENT ROADMAP', bg_color=(5, 150, 105), text_color=(255, 255, 255), width=42, height=6)
    ...
    # Milestones data
    milestones = [
        ("Phase 1: Program & Interview Prep", "Complete the Deep Bio founder interview, finalize research targets, and configure entities.", 72),
        ...
        ("Phase 3: Seed Round & IND Enabler", "Raise a $1.5M seed round to compile preclinical safety data and initiate IND filing.", 144)
    ]
    ...
    for title, desc, y_pos in milestones:
        ...
        pdf.set_font(pdf.get_font_name(True), 'B', 10.5)
        pdf.set_text_color(15, 23, 42)
