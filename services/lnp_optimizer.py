...
class LNPOptimizerService:
    ...
    def evaluate_formulation(self, molar_ratios: Dict[str, float], np_ratio: float) -> Dict[str, Any]:
        ...
        Inputs:
        - molar_ratios: Dictionary of lipid molar percentages (ionizable, helper, cholesterol, peg)
        ...
        # 2. Tissue tropism score calculations
        # Specific organ targeting coefficients multiplied by normalized molar fractions
        heart_raw = (f_ion * 3.5) + (f_chol * 1.0) + (f_helper * 0.4) - (f_peg * 20.0)
        liver_raw = (f_ion * -2.0) + (f_chol * 0.2) + (f_helper * 0.5) - (f_peg * 0.1)
