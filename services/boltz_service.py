...
def download_boltz_pdb_content(boltz_prediction_id: str, sample_index: int = 0) -> bytes:
    ...
    header = (
        "REMARK 200 COMPUTED STRUCTURE GENERATED BY NILUS LAB ZENITH PLATFORM\n"
        "REMARK 200 ENGINE: BOLTZ-1 (MIT LICENSE) / ESMFOLD OPEN WEIGHTS\n"
    )
    if not pdb_text.startswith("REMARK 200"):
        pdb_text = header + pdb_text
