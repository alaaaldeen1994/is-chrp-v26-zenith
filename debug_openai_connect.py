...
async def test_connect():
    ...
    if not key:
        ...
        print(f"✅ OPENAI_API_KEY found (len={len(key)})")
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            if resp.status_code == 200:
                print("   ✅ HIT (Authenticated)")
            elif resp.status_code == 401:
                print("   ⚠️ HIT (Auth Failed - Key invalid)")
            else:
                print(f"   ⚠️ HIT (Other Status: {resp.status_code})")
    ...
    try:
        ...
        print(f"   ✅ SDK Success: Found {len(models.data)} models")
    except Exception as e:
